...
const department = (record) =>
  ...
  clean(record.hrc_departments_hrc_employees_1_name);
const phone = (record) =>
  clean(record.phone_mobile) || clean(record.phone_work);
const columns = [
    ...
    : key === "department"
      ...
      : key === "phone"
        ...
        : key === "joining_date"
          ...
          : clean(record[key]);
const buttonClass =
  "inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 disabled:opacity-50";
...
export default function EmployeesPage() {
  ...
  const departments = [
    ...new Set(records.map(department).filter(Boolean)),
  ].sort();
  const visible = records
    .filter(
      (record) =>
        (!filter || department(record) === filter) &&
        [
          employeeName(record),
          record.email1,
          phone(record),
          ...
          clean(item).toLowerCase().includes(search.trim().toLowerCase()),
        ),
    )
    .sort(
      (a, b) =>
    ...
    <main className="min-h-full space-y-6 rounded-2xl bg-slate-50 p-3 sm:p-6">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-3 text-sm text-slate-300">
              Find your people, review their information and keep employee
              records up to date.
            </p>
          </div>
          <div className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 px-6 py-4">
            ...
            <div>
              <p className="text-3xl font-semibold">
                {query.isPending ? "—" : records.length.toLocaleString()}
              </p>
              <p className="mt-1 text-xs text-slate-300">Total employees</p>
            </div>
          </div>
        </div>
      </header>
      ...
      >
        ...
        <div className="flex flex-wrap gap-3 p-5">
          <label className="flex min-w-52 flex-1 items-center gap-2 rounded-xl border border-slate-200 px-3">
        ...
        {query.isError && (
          <p
            role="alert"
            className="mx-5 mb-4 rounded-xl bg-red-50 p-3 text-sm text-red-700"
          >
            {query.error.message}{" "}
        ...
        {query.isPending ? (
          <p
            role="status"
            className="flex justify-center gap-2 p-16 text-slate-500"
          >
                ...
                {rows.map((record, index) => (
                  ...
                  >
                     <td className="px-5 py-4 text-slate-600">
                      {clean(record.joining_date) || "—"}
                    </td>
                    <td className="px-5 py-4">
                      <button
                        disabled={!record.id}
                        onClick={() => setSelection({ id: record.id })}
                        className="flex items-center gap-3 text-left font-semibold text-slate-900"
                      >
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600">
                          {employeeName(record).slice(0, 1).toUpperCase()}
                        </span>
                        <span>
                          {employeeName(record)}
                          <span className="mt-1 block text-xs font-normal text-slate-500">
                            {clean(record.current_designation) ||
                              clean(record.title) ||
                              "View profile"}
                          </span>
                        </span>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-1">
                        <button
                          aria-label={`View ${employeeName(record)}`}
                          title="View details"
                          disabled={!record.id}
                          onClick={() => setSelection({ id: record.id })}
                          className={buttonClass}
                        ...
                        <button
                          aria-label={`Edit ${employeeName(record)}`}
                          title="Edit employee"
                          disabled={!record.id}
                          onClick={() =>
                            setSelection({ id: record.id, edit: true })
                          }
                          className={buttonClass}
                        >
                          <Pencil size={15} />
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-5 py-4 text-sm text-slate-500">
          <p>
            {visible.length ? (currentPage - 1) * 20 + 1 : 0}–
            {Math.min(currentPage * 20, visible.length)} of {visible.length}{" "}
            employees
          </p>
