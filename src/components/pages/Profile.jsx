...
function CandidateProfile({ email, jobId }) {
  ...
  useEffect(() => {
    ...
    fetchCandidateRelated(record.id, section)
      .then((items) => {
        if (active) {
          relatedCache.current[section] = { items, original: items };
          setRelated((prev) => ({
            ...prev,
            [section]: relatedCache.current[section],
          }));
        }
      })
      .catch((err) => {
  ...
  const parseResume = async (event) => {
    ...
    try {
      if (!FETCH_GPC_X_API_KEY)
        throw new Error(
          "Resume upload is not configured. Please contact your administrator.",
        );
      ...
      const response = await crmFetch(
        "https://test.hrcrm.ai/index.php?entryPoint=hrc&type=resume_parsing",
        {
          method: "POST",
      ...
      if (!candidate)
        throw new Error(
          "Resume parsed, but your candidate profile is not available yet. Reload your profile to retry the lookup.",
        );
  ...
  const save = async (event) => {
    ...
    try {
      ...
      const saved = await saveCandidate(
        existing?.id,
        { ...draft, candidate_source: existing?.candidate_source },
        email,
      );
      if (!alive.current) return;
      if (!saved.id) {
        ...
        if (!confirmed)
          throw new Error(
            "Save was accepted, but the profile could not be confirmed. Retry saving to check again.",
          );
      ...
      for (const [key, entry] of Object.entries(relatedCache.current)) {
        for (const item of entry.items || []) {
          ...
          entry.original = entry.original.map((value) =>
  ...
  const tabs = [
    {
      ...
      label: "Personal info",
      icon: UserRound,
      subtitle: "The person behind the profile",
      description: "Your introduction, contact details and professional links.",
    },
    {
      id: "education",
      label: "Education",
      ...
      description:
        "Your qualifications, institutions and academic achievements.",
    },
    {
      id: "experiences",
      label: "Experience",
      icon: BriefcaseBusiness,
      subtitle: "The work that shaped you",
      description: "Your roles, responsibilities and career milestones.",
    },
    {
      id: "skills",
      label: "Skills",
      ...
      description: "Highlight your strengths and the tools you know best.",
    },
    ...(record?.id
      ? [
          {
  ...
  const cancelEditing = () => {
    relatedCache.current = Object.fromEntries(
      Object.entries(relatedCache.current).map(([key, entry]) => [
        key,
        { ...entry, items: entry.original },
      ]),
    );
    ...
    <main className="min-h-full bg-slate-50/70 px-4 py-6 sm:px-8 sm:py-8">
      <div className="w-full space-y-6">
        ...
        {notice && (
          <div
            role="status"
            className="flex flex-wrap items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm text-emerald-800"
          >
            <CheckCircle2 size={17} className="shrink-0" />
            <span className="flex-1">{notice}</span>
            {record?.id && !draft?.profile_image && !editing && (
              <button
                onClick={() => {
                  setPhase("edit");
                  setSection("photo");
          ...
          <>
            <header className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              <div className="relative h-28 overflow-hidden bg-gradient-to-r from-sidebar-primary to-sidebar-secondary sm:h-32">
                <div className="absolute -right-10 -top-44 h-96 w-96 rounded-full border-[48px] border-white/5" />
                <div className="absolute right-52 top-8 h-56 w-56 rounded-full border border-white/10" />
                <span className="absolute left-6 top-6 text-[10px] font-medium uppercase tracking-[0.25em] text-white/70 sm:left-8">
              ...
              <div className="relative px-6 pb-6 sm:px-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                  <div className="-mt-10 flex flex-col items-start gap-4 sm:flex-row sm:items-end">
                    <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-3xl border-[5px] border-white bg-indigo-50 text-3xl font-semibold text-indigo-500 shadow-sm">
                      {photoSrc ? (
                        <img
                          key={photoSrc}
                          src={photoSrc}
                    ...
                    <div className="min-w-0 sm:pb-1">
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        {draft.current_designation || "Ready for what's next"}
                      </p>
                    </div>
                  </div>
                  {!editing ? (
                ...
                <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 border-t border-slate-100 pt-4 text-xs text-slate-500">
                  ...
                  {draft.primary_address_city && (
                    <span className="inline-flex items-center gap-2">
                      <MapPin size={14} className="text-slate-400" />
                      {[
                        draft.primary_address_city,
                        draft.primary_address_country,
                      ]
                        .filter(Boolean)
                        .join(", ")}
                    </span>
            ...
            <div className="grid items-start gap-6 lg:grid-cols-[240px_minmax(0,1fr)]">
              <aside className="space-y-5 lg:sticky lg:top-5">
                <nav
                  ...
                  className="flex gap-1 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 lg:flex-col"
                >
                  {tabs.map((tab, index) => (
                    <button
                      key={tab.id}
                      type="button"
                      aria-current={section === tab.id ? "step" : undefined}
                      aria-controls="profile-section"
                      disabled={busy || photoBusy}
                      onClick={() => goTo(tab.id)}
                      className={`flex shrink-0 items-center gap-3 rounded-xl px-4 py-3.5 text-left text-sm transition disabled:opacity-50 ${section === tab.id ? "bg-indigo-50 font-semibold text-indigo-700" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
                    >
                      <tab.icon size={18} />
                      ...
                      <span
                        className={`hidden text-[10px] lg:block ${section === tab.id ? "text-indigo-400" : "text-slate-300"}`}
                      >
                ...
                <div className="hidden rounded-2xl border border-slate-200 bg-white p-5 lg:block">
                  ...
                  <div
                    ...
                    aria-valuenow={Math.round((completed / 8) * 100)}
                    aria-valuemin={0}
                    aria-valuemax={100}
                  ...
                  <p className="mt-3 text-xs leading-5 text-slate-400">
                    Small details make a big impression. Let your profile
                    reflect you.
                  </p>
                </div>
              </aside>
              ...
              >
                <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-6 sm:px-8">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-indigo-500">
                      {editing ? "Review your story" : "Get to know me"} /{" "}
                  ...
                  <span className="hidden rounded-xl bg-slate-50 p-3 text-slate-400 sm:block">
                    <activeTab.icon size={22} />
                  </span>
                </div>
                {section === "photo" && record?.id ? (
                    ...
                    {editing ? (
                      ...
                      <div className="py-8 text-center">
                        ...
                        <p className="mt-4 text-sm text-slate-500">
                          {draft.profile_image
                            ? "Your profile photo is up to date. Want a fresh look?"
                        ...
                        >
                          {draft.profile_image ? "Change photo" : "Add photo"}
                        </button>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    <fieldset disabled={busy || photoBusy}>
                      {candidateRelatedModules[activeTab.id] ? (
                        related[activeTab.id]?.loading ? (
                          <p role="status" className="text-sm text-slate-500">
                            Loading {activeTab.label.toLowerCase()}...
                          </p>
                        ) : related[activeTab.id]?.error ? (
                          ...
                          <ProfileSectionContent
                            ...
                            setDraft={(updater) => {
                              ...
                              const updated = updater({
                                ...draft,
                                resumeSections: { [key]: entry.items },
                              });
                              relatedCache.current[key] = {
                                ...entry,
                                items: updated.resumeSections[key],
                              };
                              setRelated((prev) => ({
                                ...prev,
                                [key]: relatedCache.current[key],
                              }));
                            }}
                            editing={editing}
                          />
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 bg-slate-50/60 px-6 py-5 sm:px-8">
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      disabled={activeIndex === 0 || busy || photoBusy}
                    ...
                    {activeIndex < tabs.length - 1 && (
                      <button
                        ...
                        disabled={busy || photoBusy}
                        onClick={() => goTo(tabs[activeIndex + 1].id)}
                        className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs font-semibold text-indigo-600 hover:bg-white disabled:opacity-50"
                      >
                        Next
                        <ArrowRight size={15} />
                      </button>
                  ...
                  {editing ? (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="rounded-lg px-3 py-2 text-sm text-slate-500 hover:bg-white disabled:opacity-50"
                      >
                        Cancel
                      </button>
                      <button
                        type={section === "photo" ? "button" : "submit"}
                        form={
                          section === "photo" ? undefined : "candidate-details"
                        }
                        onClick={section === "photo" ? save : undefined}
                        disabled={busy || photoBusy}
