...
import { candidateFields } from "./candidateApi";
...
const personalGroups = [
  ...
  { title: "Your professional introduction", keys: ["current_designation", "department", "linkedin_url", "github_url", "description"] },
];
const templates = {
  education: { institution: "", degree: "", field_of_study: "", start_date: "", end_date: "", description: "" },
  experiences: { company: "", designation: "", location: "", start_date: "", end_date: "", description: "" },
...
export default function ProfileSectionContent({ section, draft, setDraft, editing }) {
  if (section === "personal") return <div className="space-y-8">{personalGroups.map(group => <section key={group.title}>
    ...
    <div className="grid gap-x-5 gap-y-5 sm:grid-cols-2">{group.keys.map(key => <div key={key} className={key === "description" || key === "primary_address_street" ? "sm:col-span-2" : ""}>
  ...
  const add = () => setDraft(prev => ({ ...prev, resumeSections: { ...prev.resumeSections, [section]: [...(prev.resumeSections?.[section] || []), { ...templates[section] }] } }));
  return <div className="space-y-4">
    {!items.length && <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-6 py-12 text-center"><Icon className="mx-auto mb-4 text-indigo-400" size={32} /><h3 className="font-medium text-slate-800">Room for your {singular}</h3><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-slate-500">{editing ? `No ${singular} details added yet. Add an entry to complete this part of your story.` : `Your ${singular} details will appear here once you add them.`}</p></div>}
    {items.map((item, index) => {
      ...
      return <article key={`${section}-${index}`} className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        <div className="mb-5 flex items-start gap-3"><span className="rounded-xl bg-indigo-50 p-2.5 text-indigo-600"><Icon size={20} /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-400">{singular} / {String(index + 1).padStart(2, "0")}</p><h3 className="mt-1 break-words font-semibold text-slate-900">{title}</h3></div>{editing && <button type="button" aria-label={`Remove ${singular} ${index + 1}`} onClick={() => setDraft(prev => ({ ...prev, resumeSections: { ...prev.resumeSections, [section]: items.filter((_, i) => i !== index) } }))} className="rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 size={16} /></button>}</div>
        <div className="grid gap-4 sm:grid-cols-2">{Object.entries(item).map(([key, value]) => <div key={key} className={/description|details|summary|responsibilit/i.test(key) ? "sm:col-span-2" : ""}>
    ...
    {editing && <button type="button" onClick={add} className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-indigo-200 bg-indigo-50/40 px-4 py-3 text-sm font-medium text-indigo-600 hover:bg-indigo-50"><Plus size={17} />Add {singular}</button>}
  </div>;
}
