...
export default function TodayAttendanceCard() {
  ...
  const record = useMemo(
    () =>
      Array.isArray(data)
        ? data[0] || null
        : data?.records?.[0] || data?.data?.[0] || null,
    [data],
  );
    ...
    return (
      <section className="employee-card today-card">
        ...
        <div className="attendance-error">
          <Fingerprint size={32} />
          <p>Unable to load today's attendance.</p>
  ...
  if (!isPresentToday)
    return (
      <section className="employee-card today-card">
        <div className="card-title-row">
          <h2 className="card-title">Today</h2>
          <span className="status-badge">Absent</span>
        </div>
        <div className="divider" />
        <div className="today-card__body">
          <div>
            <Fingerprint className="today-card__icon" size={27} />
          ...
          <div className="progress-ring" style={{ "--progress": 0 }}>
